...
/**
 * This class is a data manager that initializes data of cheese factory and returns desired data
 * upon requests.
 */
public class DataManager {
        ...
        if (farmMonthlyReport(year)[i].farmID.equals(farmID)) {
          for (int j = 0; j < farmMonthlyReport(year)[i].monthlyReport.length; j++) {
            writer.println("Month " + (j + 1) + " Weight: "
                + farmMonthlyReport(year)[i].monthlyReport[j] + " Percent of total milk for month: "
  ...
  /**
   * This returns a vector of length 12 representing the average weight of milk produced by ALL
   * FARMS in each month of the year
   * 
   * @param year
  ...
  /**
   * This returns a vector of length 12 representing the weight of milk produced by ALL FARMS in
  ...
  private farmMonthlyReport[] farmMonthlyReport(int year) {
    ...
    for (int i = 0; i < numberOfFarms; i++) {
      ...
      farmReportByMonth[i] = new farmMonthlyReport(farm.getFarmID(), farm.monthlySum(year));
    }
   ...
   * @param farmID farm identifier
   * @param year
   * @return a vector of length 12 representing the average amount of milk produced by this farm in
  ...
  /**
   * Return the maximum daily production of each month for the farm and year specified.
   * 
   * @param farmID farm identifier
   * @param year   year
   * @return a vector of length 12 representing maximum daily production of each month
   */
   ...
   * @param farmID farm identifier
   * @param year   year
   * @return a vector of length 12 representing minimum daily production of each month
   */
   ...
   * @param farmID farm identifier
   * @param year
   * @return a vector of length 12 representing the total amount of milk produced by this farm every
   *         month of the year
   */
  ...
  /**
   * Return the percentage of milk that a farm contributed(with respect to all farms) on monthly
   * basis.
   * 
   * @param year
   * @param month
  ...
  /**
   * Return the percentage of milk that a farm contributed(with respect to all farms) in the year
   * specified.
   * 
   * @param year
   * @return
  ...
  /**
   * Return the percentage of milk that a farm contributed(with respect to all farms) throughout the
   * time.
   * 
   * @return
   */
  ...
  /**
   * Calculate the average amount of milk (i.e per day) produced by the farm during a range of time.
   * 
   * @param farmID farm identifier
   ...
   * @return the average amount of milk (i.e per day) produced by the farm during a range of time
   */
  public double getAvgInRangeForFarm(String farmID, int yStart, int mStart, int dStart, int yEnd,
      int mEnd, int dEnd) {
    ArrayList<Farm> listOfFactories = factory.getFarmList();
    for (Farm f : listOfFactories) {
      if (f.getFarmID().equals(farmID))
   ...
   */
  public int getSumInRangeAllFarms(int yStart, int mStart, int dStart, int yEnd, int mEnd,
      int dEnd) {
    int sum = 0;
    ArrayList<Farm> listOfFactories = factory.getFarmList();
    for (Farm f : listOfFactories) {
   ...
   */
  public double[] getSumInRangeEachFarm(int yStart, int mStart, int dStart, int yEnd, int mEnd,
      int dEnd) {
    ...
    int i = 0;
    for (Farm f : listOfFactories) {
      sumInRangeEachFarm[i++] =
          (double) f.sumWithinRange(yStart, mStart, dStart, yEnd, mEnd, dEnd)[0];
    }
   ...
   */
  public double getMinInRange(int yStart, int mStart, int dStart, int yEnd, int mEnd, int dEnd) {
    double[] sumInRangeEachFarm = getSumInRangeEachFarm(yStart, mStart, dStart, yEnd, mEnd, dEnd);
   ...
   */
  public double getMaxInRange(int yStart, int mStart, int dStart, int yEnd, int mEnd, int dEnd) {
    double[] sumInRangeEachFarm = getSumInRangeEachFarm(yStart, mStart, dStart, yEnd, mEnd, dEnd);
   ...
   */
  public double[] getPercentageInRangeAllFarms(int yStart, int mStart, int dStart, int yEnd,
      int mEnd, int dEnd) {
    int sumInRangeAllFarms = getSumInRangeAllFarms(yStart, mStart, dStart, yEnd, mEnd, dEnd);
    double[] sumInRangeEachFarm = getSumInRangeEachFarm(yStart, mStart, dStart, yEnd, mEnd, dEnd);
    int factorySize = sumInRangeEachFarm.length;
    double[] percentgeInRangeEachFarm = new double[factorySize];
    for (int i = 0; i < factorySize; i++)
      percentgeInRangeEachFarm[i] = sumInRangeEachFarm[i] / sumInRangeAllFarms * 100;
